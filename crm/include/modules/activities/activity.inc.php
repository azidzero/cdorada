...
switch ($o) {
                            ...
                            <tr>
                                <td>
                                    <div class="input-group">
                                        ...
                                        <select id="category" name="category" class="form-control" onchange="chkMode()">
    ...
    // default is 0, no entry will be selected.
                                                db_createlist($CNN,0,
                                                        "select nom from (select name as nom from crm_company UNION select name from crm_contact) as g","Sleccione una persona...");
                                            ?>
                                        </select>
                                    </div>
                                </td>
	...
	mysqli_stmt_free_result($stmt1);
        ...
        mysqli_query($CNN, "INSERT INTO crm_activities (title, category, idCompanyOrPerson, dateActivity, timeActivity, description, typeCompanyOrPerson, activo) values('$title', '$tipoC', $dato, '$datepicker', '$taskTime', '$description', '$tipoD', 'SI')");
