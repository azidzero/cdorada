...
switch ($o) {
    case 0:
        ?>
        <form action="./?m=warehouse&s=cat&o=1" method="post">
            <h4>Categor&iacute;a de Producto</h4>
            <table class="table table-condensed">
                <tr>
                    <td>
                        <div class="input-group">
                            <span class="input-group-addon">Categor&iacute;a Padre:</span>
                            <select id="parent" name="parent" class="form-control">
                                <option value="0">Ninguna</option>
                                <?php
                                $q = mysqli_query($this->C->CNN, "SELECT * from warehouse_cat");
        ...
        break;
    case 1:
        $code = $_REQUEST["parent"];
        $name = $_REQUEST["name"];
        mysqli_query($this->C->CNN, "INSERT INTO warehouse_cat(parent,name) VALUES('$code','$name')") or $e = mysqli_errno() . ": " . mysqli_error();
        if (!isset($e)) {
            ?>
            <div class="alert alert-success">
                <h4>Se ha agregado la categor&iacute;a <?php echo $name; ?> al sistema</h4>
            </div>
            <?php
        } else {
            ?>
            <div class="alert alert-danger">
                <h4>No se ha agregado la categor&iacute;a <?php echo $name; ?> al sistema</h4>
                <p><?php echo $e; ?></p>
            </div>
            <?php
        }
        ...
        <div class="well well-sm">
            <div class="btn-group">
                <a href="./?m=warehouse&s=cat&o=0" class="btn btn-default">Agregar Nueva &Aacute;rea</a>
                <a href="./?m=warehouse&s=cat&o=2" class="btn btn-default">Ir a adminstrar</a>
            </div>
        </div>
        <?php
        break;
    case 2:
        ...
        <script>
            $(document).ready(function () {
                jTable('tbl', 'include/modules/warehouse/cat.table.php');
            });
        </script>
        <?php
        break;
    case 3:
        ...
        while ($r = mysqli_fetch_array($q)) {
            ?>
            <form action="./?m=warehouse&s=cat&o=4" method="post">
