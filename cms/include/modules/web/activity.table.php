...
$sIndexColumn = "title";
$sTable = "web_activity_translate a";
$gaSql['user'] = DBU;
$gaSql['password'] = DBP;
$gaSql['db'] = DBB;
$gaSql['server'] = DBH;
...
$sQuery = "SELECT SQL_CALC_FOUND_ROWS " . str_replace(" , ", " ", implode(", ", $aColumns)) . " FROM $sTable INNER JOIN web_activity_category b ON(a.category=b.id) where a.lang='es' group by uid $sWhere $sOrder $sLimit";
...
while ($aRow = mysqli_fetch_array($rResult)) {
    ...
    $row[4] .= "<li><a href=\"./?m=web&s=activity&o=3&id={$aRow['uid']}\"><i class=\"fa fa-edit\"></i>Editar</a></li>";
    $row[4] .= "<li><a href=\"./?m=web&s=activity&o=5&id={$aRow['uid']}\"><i class=\"fa fa-trash\"></i> Eliminar</a></li>";
    $row[4] .= "</ul>";
    $row[4] .= "</div>";
    $output['aaData'][] = $row;
}
