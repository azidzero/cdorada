...
switch ($op) {
    //GUARDA LA CATEGORIA
    case 0:
        $QRY = "insert into web_empre_category (name,lang)values('" . filter_input(INPUT_POST, "namcat_es") . "','es')";
        $EXE = mysqli_query($CNN, $QRY)or $e = "Error al insertar la categoria en <b>'es'</b>" . mysqli_error($CNN);
        if (!isset($e)) {
            $pad = mysqli_insert_id($CNN);
            $ol = 0;
            $qry = "insert into web_empre_category (pid,name,lang)values";
            foreach ($lgg as $g) {
                if ($g != 'es') {
    ...
    case 1:
        ...
        $getc = "select * from web_empre_category where id=$id or pid=$id";
    ...
    case 2:
        ...
        $q = "select * from web_empre_category where id=$id or pid=$id";
        ...
        if (count($resultado) >= 1) {
            ...
            $mltqry = "update web_empre_category set name='" . filter_input(INPUT_POST, "namcat_es") . "' where id=$id";
            $uplang = mysqli_query($CNN, $mltqry) or $e = mysqli_error($CNN);
            if (!isset($e)) {
                $del = "delete from web_empre_category where pid=$id";
    ...
    case 3:
        $id = filter_input(INPUT_POST, "cid");
        $noro = mysqli_query($CNN, "select * from web_empre where cat=$id") or $err = "Error al eliminar la categoria<br>" . mysqli_error($CNN);
        if (!isset($err)) {
            $nc = mysqli_num_rows($noro);
            if ($nc <= 0) {
                $elimall = "delete from web_empre_category where id=$id";
    ...
    case 80:
        ...
        $empre = filter_input(INPUT_POST, "empre_id");
        $q = mysqli_query($CNN, "SELECT * from web_empre_gallery WHERE id=$id and aid=$empre") or die(mysqli_error($CNN));
        while ($r = mysqli_fetch_array($q)) {
            unlink("../../../content/upload/empre" . $r['name'] . "_m.jpg");
            unlink("../../../content/upload/empre" . $r['name'] . "_b.jpg");
        }
        $del = mysqli_query($CNN, "DELETE from web_empre_gallery where aid=$empre and id=$id ")OR DIE(mysqli_error($CNN));
        if (!$del) {
            ?>
