...
switch ($o) {
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-group">
                            <select id="post-cat" name="post-cat" class="form-control">
                                <option disabled selected><?php echo $lang->getString("activity", "post-cat-default"); ?></option>
                                <?php
                                $oq = mysqli_query($CNN, "SELECT * from web_activity_category where pid=0");
                                    ...
                                    <div class="form-group">
                                        <div class="input-group">
                                            <span class="input-group-addon">
                                                ...
                                                <strong><?php echo $lang->getString("activity", "post-content"); ?></strong></span>
            ...
            <div class="modal-dialog modal-lg" style="width:20%;">
                <div class="modal-content">
                    ...
                    <div class="modal-body" >
                        ...
                        <button type="text" class="btn btn-danger" onclick="dodellcat()">Eliminar</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    case 1:
        ...
        mysqli_query($CNN, "INSERT INTO web_activity(fecha,hora,cat) VALUES('$date','$time','$categ')") or $e = mysqli_error($CNN);
        if (!isset($e)) {
            ...
            foreach ($language as $L) {
                unset($le);
                mysqli_query($CNN, "INSERT INTO web_activity_translate(uid,lang,title,short_desc,content)
                ...
                <div class="col-lg-4">
                    <div class="input-group">
                        ...
                        <div class="input-group">
                            <select id="post-cat" name="post-cat" class="form-control">
                                <?php
                                $getcat = mysqli_query($CNN, "select * from web_activity where id=$id")or $err = mysqli_error($CNN);
                                $ct = 0;
                                ...
                                ?>><?php echo $lang->getString("activity", "post-cat-default"); ?></option>
                                        <?php
                                        $oq = mysqli_query($CNN, "SELECT * from web_activity_category WHERE pid=0");
                                        while ($or = mysqli_fetch_array($oq)) {
                        ...
                        ?>" id="tab_<?php echo $L; ?>">
                            <div class="row">
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        <input type="hidden" id="post-content-<?php echo $L; ?>" name="post-content-<?php echo $L; ?>" />
                                        <span class="input-group-addon"> <strong><?php echo $lang->getString("activity", "post-content"); ?></strong></span>
    ...
    case 4:
        ...
        $upd=  mysqli_query($CNN, "update web_activity_translate set category=$cat where uid=$id");
        $lq = mysqli_query($CNN, "SELECT * from cms_translation_lang WHERE status=1") or die(mysqli_error($CNN));
        $language = array();
        while ($lr = mysqli_fetch_array($lq)) {
            $language[] = $lr['iso_639_1'];
    ...
    case 6:
        ...
        $getima=  mysqli_query($CNN, "select * from web_activity_gallery where aid=$id ")or $e=mysqli_error($CNN);
        while($ii=  mysqli_fetch_array($getima))
        {
           unlink("content/upload/activity/".$ii['name']."_b.jpg");
           unlink("content/upload/activity/".$ii['name']."_m.jpg");
        }
        mysqli_query($CNN, "DELETE FROM web_activity_gallery WHERE aid='$id'") or $e = mysqli_error($CNN);
        if (!isset($e)) {
            ?>
