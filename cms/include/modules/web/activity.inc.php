...
switch ($o) {
    ...
    case 1:
        ...
        $date = date("Y-m-d");
        mysqli_query($CNN, "INSERT INTO web_activity(title,short_desc,content,category,fecha) 
            VALUES('$title','$short_desc','$content','$cat','$date')") or die(mysqli_error($CNN));
        $id = mysqli_insert_id($CNN);
        $des = "content/activity/item_" . str_pad($id, 6, "0", STR_PAD_LEFT) . ".jpg";
    ...
    case 3:
        ...
        $q = mysqli_query($CNN, "SELECT * from web_activity WHERE id=$id");
    ...
    case 4:        
        ...
        $cat = filter_input(INPUT_POST, "post-cat");
        $content = mysqli_real_escape_string($CNN, filter_input(INPUT_POST, "post-content"));
        mysqli_query($CNN, "UPDATE web_activity SET 
