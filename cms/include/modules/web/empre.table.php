...
$sQuery = "SELECT SQL_CALC_FOUND_ROWS " . str_replace(" , ", " ", implode(", ", $aColumns)) . " FROM $sTable INNER JOIN web_empre_category b ON(a.category=b.id) where a.lang='es' group by uid $sWhere $sOrder $sLimit";
...
while ($aRow = mysqli_fetch_array($rResult)) {
    ...
    $row[4] .= "<li><a href=\"./?m=web&s=empre&o=3&id={$aRow['uid']}\"><i class=\"fa fa-edit\"></i>Editar</a></li>";
    $row[4] .= "<li><a href=\"./?m=web&s=empre&o=5&id={$aRow['uid']}\"><i class=\"fa fa-trash\"></i> Eliminar</a></li>";
    $row[4] .= "</ul>";
    $row[4] .= "</div>";
    $output['aaData'][] = $row;
}
