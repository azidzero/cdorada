...
switch ($op) {
    //GUARDA LA CATEGORIA
    case 0:
        $QRY = "insert into web_info_category (name,lang)values('" . filter_input(INPUT_POST, "namcat_es") . "','es')";
        $EXE = mysqli_query($CNN, $QRY)or $e = "Error al insertar la categoria en <b>'es'</b>" . mysqli_error($CNN);
        if (!isset($e)) {
            $pad = mysqli_insert_id($CNN);
            $ol = 0;
            $qry = "insert into web_info_category (pid,name,lang)values";
            foreach ($lgg as $g) {
                if ($g != 'es') {
    ...
    case 1:
        ...
        $getc = "select * from web_info_category where id=$id or pid=$id";
    ...
    case 2:
        ...
        $q = "select * from web_info_category where id=$id or pid=$id";
        ...
        if (count($resultado) >= 1) {
            ...
            $mltqry = "update web_info_category set name='" . filter_input(INPUT_POST, "namcat_es") . "' where id=$id";
            $uplang = mysqli_query($CNN, $mltqry) or $e = mysqli_error($CNN);
            if (!isset($e)) {
                $del = "delete from web_info_category where pid=$id";
    ...
    case 3:
        $id = filter_input(INPUT_POST, "cid");
        $noro = mysqli_query($CNN, "select * from web_info where cat=$id") or $err = "Error al eliminar la categoria<br>" . mysqli_error($CNN);
        if (!isset($err)) {
            $nc = mysqli_num_rows($noro);
            if ($nc <= 0) {
                $elimall = "delete from web_info_category where id=$id";
    ...
    case 80:
        ...
        $q = mysqli_query($CNN, "SELECT * from web_info_gallery WHERE id=$id and aid=$prop") or die(mysqli_error($CNN));
        while ($r = mysqli_fetch_array($q)) {
            unlink("../../../content/upload/info/" . $r['name'] . "_m.jpg");
            unlink("../../../content/upload/info/" . $r['name'] . "_b.jpg");
        }
        $del = mysqli_query($CNN, "DELETE from web_info_gallery where aid=$prop and id=$id ")OR DIE(mysqli_error($CNN));
        if (!$del) {
            ?>
